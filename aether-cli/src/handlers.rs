...
pub async fn handle_broker_start(
    api_port: usize,
    jrpc_port: usize,
...
pub async fn handle_task_check(
    broker_ip: Option<String>,
    broker_api_port: Option<usize>,
    task_id: String,
    token: Option<String>,
    ...
    if let Some(err) = response.error {
        ...
        let de_task =
            serde_json::to_string_pretty(&task).map_err(|_| CliError::DeserializeTaskError)?;
        println!("{de_task}");
    }
    Ok(())
}
...
pub async fn handle_task_list(
    ...
    .await?;
    let de_tasks =
        serde_json::to_string_pretty(&response).map_err(|_| CliError::DeserializeTaskListError)?;
    println!("{de_tasks}");
    Ok(())
}
...
pub async fn handle_auth_switch(profile: String) -> Result<(), CliError> {
    let mut cfg = AetherConfig::get()?;
...
pub async fn handle_auth_logout(profile: String) -> Result<(), CliError> {
    let mut cfg = AetherConfig::get()?;
